...
class SEGating2d(nn.Module):
    def __init__(self, channel, reduction=16):
        super(SEGating2d, self).__init__()
...
class UNet_3D_3D(nn.Module):
    def __init__(self, block , n_inputs=4, n_outputs=3, batchnorm=False , joinType="concat" , upmode="transpose"):
        ...
        unet_3D = importlib.import_module(".resnet_3D" , "model_inter")
        if n_outputs > 1:
        ...
        self.feature_fuse = Conv_2d(nf[3]*n_inputs , nf[3]*n_inputs , kernel_size=1 , stride=1, batchnorm=batchnorm)
        self.fg1=SEGating2d(nf[3]*n_inputs)
    ...
    def forward(self, images):
        ...
        dx_0 = joinTensors(dx_0 , x_0 , type=self.joinType)   # [1,128,4,256,256]

        # print(dx_3.shape,dx_2.shape,dx_1.shape,dx_0.shape)
        dx_out = self.lrelu(self.decoder[4](dx_0))     # [1,64,4,512,512]
        # print(dx_3.shape,dx_2.shape,dx_1.shape,dx_0.shape,dx_out.shape)
        # exit()

        dx_out = torch.cat(torch.unbind(dx_out , 2) , 1)   # [1,256,512,512]
        dx_out=self.feature_fuse(dx_out)
        dx_out=self.fg1(dx_out)

        # out = self.lrelu(self.feature_fuse(dx_out))        # [1,64,512,512]
        # out = self.outconv(out)                            # [1,21,512,512]
        ...
        return dx_out,dx_3,dx_2,dx_1,dx_0
